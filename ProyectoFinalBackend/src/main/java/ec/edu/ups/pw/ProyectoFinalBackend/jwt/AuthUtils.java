...
public final class AuthUtils {
	...
	public static Token createToken(String host, User user) throws JOSEException {
		JWTClaimsSet claim = new JWTClaimsSet.Builder().subject(user.getEmail()).issuer(host)
				.issueTime(DateTime.now().toDate()).expirationTime(DateTime.now().plusDays(1).toDate())
				.claim("user", user.getEmail()).claim("role", user.getRol()).build();
